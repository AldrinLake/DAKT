...
from utils.data import iCIFAR10, iCIFAR100, iImageNet100, iTinyImageNet200, iCUB200, iCARS, iImageNet1000
...
class DataManager(object):
    def __init__(self, dataset_name, shuffle, seed, init_cls, increment):
        ...
        # Shuffle changes the label ordering for each class.
    ...
    def get_dataset(self, indices, source, mode, shot=None, appendent=None, ret_data=False, m_rate=None):
        """
        m_rate: dropped-sample ratio; if None, no random sampling is applied.
        shot: number of samples per class in the training data; if None, use all data.
        """
        if source == "train":
    ...
    def _setup_data(self, dataset_name, shuffle, seed):
        """
        Populate the dataset arrays and transform pipelines.
        """
...
def pil_loader(path):
    ...
    with open(path, "rb") as f:
        try:
            ...
            print(f"[Warning] I/O error loading {path}: {e}")
