...
class BaseNet(nn.Module):
    ...
    def forward(self, x):
        x = self.convnet(x)     #   return {'fmaps': [x_1, x_2, x_3, x_4], 'features': features}
...
class IncrementalNet(BaseNet):
    ...
    def init_de(self):
        drift_estimator = SimpleLinear(self.feature_dim, self.feature_dim)
        drift_estimator.weight = nn.Parameter(torch.eye(self.feature_dim))
        drift_estimator.bias = nn.Parameter(torch.zeros(self.feature_dim))
        del self.drift_estimator
    ...
    def forward(self, x):
        x = self.convnet(x)             #return {'fmaps': [x_1, x_2, x_3, x_4], 'features': features}
        out = self.fc(x["features"])    # return {'logits': F.linear(input, self.weight, self.bias)}
        out.update(x)
        if hasattr(self, "gradcam") and self.gradcam:
    ...
    def freeze_de(self):
        for param in self.drift_estimator.parameters():
            param.requires_grad = False
        self.drift_estimator.eval()

    def unfreeze_de(self):
        for param in self.drift_estimator.parameters():
            param.requires_grad = True
        self.drift_estimator.train()
